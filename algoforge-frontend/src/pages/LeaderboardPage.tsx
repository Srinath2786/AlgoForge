                  ...
                  {[entries[1], entries[0], entries[2]].map((e, idx) => {
                    ...
                    return (
                      <div key={e.userId} className="flex flex-col items-center">
                        <div className="flex h-11 w-11 items-center justify-center rounded-full border border-forge/20 bg-gradient-to-br from-forge/15 to-cyan/10 text-[11px] font-bold text-forge">
                          {e.username.slice(0, 2).toUpperCase()}
                        </div>
                        <p className="mt-2 max-w-[7rem] truncate text-center text-xs font-medium text-ink">{e.username}</p>
                        <p className="mt-0.5 font-mono text-[11px] text-forge mono-num">{e.score.toFixed(1)}</p>
                        <div className={cn("mt-2 flex w-full items-center justify-center rounded-t-2xl border border-b-0 pt-3 shadow-inner", height, tone)}>
              ...
              <div className="space-y-3">
                {entries.map((e, i) => (
                  <motion.div
                    ...
                    className={cn(
                      "group grid grid-cols-[36px_44px_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border px-3 py-3 transition-all sm:px-4",
                      e.username === username
                  ...
                  >
                    <div className="flex items-center justify-center text-center">
                      {e.rank === 1 ? (
                        ...
                        <Medal size={16} className={rankTone(e.rank)} />
                      ) : (
                        <span className="font-mono text-xs font-semibold text-ink-faint mono-num">#{e.rank ?? "—"}</span>
                      )}
                    </div>

                    <div className="flex h-10 w-10 items-center justify-center rounded-full border border-forge/20 bg-gradient-to-br from-forge/15 to-cyan/10 text-[10px] font-bold text-forge">
                      {e.username.slice(0, 2).toUpperCase()}
                    </div>

                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="truncate text-sm font-medium text-ink">{e.username}</p>
                        {e.username === username && (
                          <span className="rounded-full border border-forge/20 bg-forge/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-forge">
                            You
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-right">
                        <p className="text-[9px] uppercase tracking-[0.14em] text-ink-faint">Score</p>
                        <p className="mt-0.5 font-mono text-sm font-semibold text-forge mono-num">{e.score.toFixed(1)}</p>
                      </div>
                      <div className="rounded-lg border border-hairline bg-elevated/60 p-1.5 text-ink-faint transition-colors group-hover:text-forge">
